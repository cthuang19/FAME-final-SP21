...
public class Ghost extends MovingAnimatedImage {
    ...
    enum GhostState {
        PASSIVE, SUSPICIOUS, ACTIVE, EXPLOSIVE
    };
    ...
    public void setGhostState(GhostState s) {
        this.state = s;
    }
    ...
    public void update(double time, Player player, ArrayList<Asteroid> asteroids) {
        ...
        updateGhostState(player, this.colour);
    ...
    /**
     * determines if the player is in the line of sight of the ghost will be used in
     * setSeesPlayer
     * 
     * @param player    the player
     * @param obstacles all the walls/asteroids that could hide the player to the
     *                  ghosts
    ...
    public boolean canSeePlayer(Player player, ArrayList<Asteroid> obstacles) {
        Rectangle2D analysis = new Rectangle2D(Math.min(player.getPositionX(), positionX),
                Math.min(player.getPositionY(), positionY), Math.abs(positionX - player.getPositionX()),
    ...
    /**
     * move the ghost and prevent it from moving on the top of asteroid
     * 
    ...
    private void updateGhostState(Player player, Ghost.Colour color) {
        double timePassed = 0;
        if (color == Colour.RED) {
            switch (state) {
            case PASSIVE:
